    ...
    where TAnalyzerDoAfterEvent : SimpleDoAfterEvent, new()
{
    ...
    {
        if (args.Target == null || !args.CanReach || !ValidScanTarget(args.Target) || !_cell.HasDrawCharge((uid.Owner, null, null), user: args.User))
            return;
    ...
    {
        if (args.Handled || args.Cancelled || args.Target == null || !_cell.HasDrawCharge((uid.Owner, null, null), user: args.User))
            return;
