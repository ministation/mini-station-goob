...
public sealed partial class HulkGenSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<GeneticsHulkComponent, ComponentInit>(OnHulkInit);
        SubscribeLocalEvent<GeneticsHulkComponent, ComponentShutdown>(OnHulkShutdown);
        SubscribeLocalEvent<GeneticsHulkComponent, HulkChargeActionEvent>(OnHulkCharge);
    }
    ...
    #region Abilities
    private void OnHulkInit(Entity<GeneticsHulkComponent> ent, ref ComponentInit args)
    {
        foreach (var action in ent.Comp.ActionPrototypes)
            ent.Comp.ActionsEntity.Add(_action.AddAction(ent, action));
    }

    private void OnHulkShutdown(Entity<GeneticsHulkComponent> ent, ref ComponentShutdown args)
    {
        foreach (var action in ent.Comp.ActionsEntity)
            _action.RemoveAction(action);
    }

    private void OnHulkCharge(Entity<GeneticsHulkComponent> entity, ref HulkChargeActionEvent args)
    {
