...
namespace Content.Server.Genetics.System
{
    ...
    public sealed partial class DnaModifierConsoleSystem : EntitySystem
    {
        ...
        {
            TryBindGeneticScanner(uid, component);
        }
        ...
        /// <summary>
        /// Bind to an already device-linked scanner, or the nearest MedicalScanner in range.
        /// Mini maps often lack pre-linked outputs; proximity keeps admin-spawned setups usable.
        /// </summary>
        private void TryBindGeneticScanner(EntityUid uid, DnaModifierConsoleComponent component)
        {
            ...
            if (TryComp<DeviceLinkSourceComponent>(uid, out var source))
            {
                ...
                {
                    if (!TryComp<MedicalScannerComponent>(port, out var linkedScanner))
                        continue;

                    component.GeneticScanner = port;
                    linkedScanner.ConnectedConsole = uid;
                    RecheckConnections(uid, component.GeneticScanner, component);
                    return;
                }
            }
            ...
            var nearestDist = component.MaxDistance;
            ...
            component.GeneticScanner = nearest;
            if (TryComp<MedicalScannerComponent>(nearest, out var nearestScanner))
                nearestScanner.ConnectedConsole = uid;
