...
namespace Content.Shared.Genetics.Systems
{
    public sealed partial class MarkingPrototypesIndexerSystem : EntitySystem
    {
        ...
        {
            // Mini/upstream MarkingPrototype uses MarkingCategory + SpeciesRestrictions (no GroupWhitelist).
            var parts = new List<string> { markingPrototype.MarkingCategory.ToString() };
            if (markingPrototype.SpeciesRestrictions is { Count: > 0 })
                parts.AddRange(markingPrototype.SpeciesRestrictions);
