...
public sealed partial class GeneticsConsoleSystem
{
    ...
    private void OnScan(Entity<GeneticsScannerComponent> ent, ref GeneticsConsoleScanMessage args)
    {
        ...
        if (!CanScan(ent.AsNullable(), mob) || _net.IsClient)
            return;

        var handheld = HasComp<HandheldGeneticsScannerComponent>(ent);
        var doAfterArgs = new DoAfterArgs(
            ...
            eventTarget: ent,
            target: handheld ? mob : null,
            used: ent)
        {
            BreakOnMove = true,
            BreakOnDamage = true,
            AttemptFrequency = AttemptFrequency.EveryTick,
            RequireCanInteract = handheld,
            NeedHand = false,
            CancelDuplicate = false,
        };
        var started = _doAfter.TryStartDoAfter(doAfterArgs);
        SetBusy(ent.AsNullable(), started);
        if (started)
            Speak(ent, "scanning");
    }
