...
namespace Content.Client.Lobby.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer
    {
        ...
        private List<(string, JobPrioritySelector)> _jobPriorities = new();

        private readonly Dictionary<string, DepartmentJobsSection> _jobCategories;
...
/*            if (configurationManager.GetCVar(GoobCVars.BarksEnabled))
            ...
            _jobCategories = new Dictionary<string, DepartmentJobsSection>();
        ...
        public void RefreshAntags()
        {
            ...
            foreach (var antag in _prototypeManager.EnumeratePrototypes<AntagPrototype>().OrderBy(a => a.LocalizedName))
            {
                ...
                if (AntagLobbyIcons.TryResolveIconPath(antag, out var iconPath) &&
                    resourceCache.TryGetResource<TextureResource>(new ResPath(iconPath), out var textureResource))
                {
                    iconTexture = textureResource.Texture;
                }
                else if (AntagLobbyIcons.TryResolveFallbackSprite(antag, out var sprite))
                {
                    iconTexture = _sprite.Frame0(sprite);
                }

                var hasLoadout = _prototypeManager.TryIndex<RoleLoadoutPrototype>(
                    LoadoutSystem.GetAntagPrototype(antag.ID), out var roleLoadoutProto);

                RoleLoadoutPrototype? capturedLoadoutProto = hasLoadout ? roleLoadoutProto : null;
                ...
                card.Setup(
                    ...
                    hasLoadout
                        ? () =>
                        {
                            ...
                            OpenLoadout(null, loadout, capturedLoadoutProto, title);
                        }
                        : null,
                    hasLoadout);

                card.Select(Profile?.AntagPreferences.Contains(antag.ID) == true ? 0 : 1);
        ...
        public void RefreshJobs()
        {
            ...
            foreach (var department in departments)
            {
                ...
                foreach (var job in jobs)
                {
                    ...
                    var hasLoadout = _prototypeManager.TryIndex<RoleLoadoutPrototype>(
                        LoadoutSystem.GetJobPrototype(job.ID), out var roleLoadoutProto);
        ...
        private Texture? GetDepartmentIcon(DepartmentPrototype department)
        {
            var jobId = DepartmentRepresentativeJobs.TryGetRepresentative(department.ID)
                ?? department.Roles.FirstOrDefault();

            if (jobId == default || !_prototypeManager.TryIndex(jobId, out var job))
                return null;

            var jobIcon = _prototypeManager.Index(job.Icon);
            return _sprite.Frame0(jobIcon.Icon);
        }
        ...
        private void OnSkinColorOnValueChanged()
        {
            ...
            switch (strategy.InputType)
            {
                case SkinColorationStrategyInput.Unary:
                {
                    ...
                    var color = strategy.FromUnary(_pixelSkinSlider.Value);
        ...
        private void UpdateJobPriorities()
        {
            ...
            foreach (var section in _jobCategories.Values)
                section.UpdateSummary(Profile);
        }
        ...
        private void UpdateSkinColor()
        {
            ...
            switch (strategy.InputType)
            {
                case SkinColorationStrategyInput.Unary:
                {
                    ...
                    _pixelSkinSlider.Value = strategy.ToUnary(Profile.Appearance.SkinColor);
                    break;
                }
                case SkinColorationStrategyInput.Color:
                {
