...
public sealed class ActionButton : Control, IEntityControl
{
    ...
    private Control? SupplyTooltip(Control sender)
    {
        ...
        // Mini: localize action name/desc (EntityName is often a locale id).
        var name = FormattedMessage.FromMarkupPermissive(LocExtensions.LocalizeOrRaw(metadata.EntityName));
        var desc = string.IsNullOrEmpty(metadata.EntityDescription)
            ? FormattedMessage.FromUnformatted(string.Empty)
            : FormattedMessage.FromMarkupPermissive(LocExtensions.LocalizeOrRaw(metadata.EntityDescription));
        ...
        if (_entities.TryGetComponent(Action, out LimitedChargesComponent? actionCharges))
        {
            ...
            chargesText = FormattedMessage.FromMarkupPermissive(
                Loc.GetString($"Charges: {charges}/{actionCharges.MaxCharges}"));
        ...
        return new ActionAlertTooltip(name, desc, charges: chargesText);
    }
