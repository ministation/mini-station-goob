reagent-name-mutadon = мутадон
reagent-desc-mutadon = Препарат, лечащий генетические болезни и сбрасывающий блоки структурных ферментов.
reagent-name-bromine = бром
reagent-desc-bromine = Ядовитый галогеновый элемент, обычно жидкий при комнатной температуре.
reagent-effect-guidebook-cure-dna-disease =
    { $chance ->
        [1] Лечит генетическую болезнь
        *[other] Лечит генетическую болезнь с шансом {NATURALPERCENT($chance, 2)} за тик
    }
reagent-effect-guidebook-mutate-dna = Случайно активирует болезненные блоки ДНК
job-description-geneticist = Открывайте мутации на ДНК-модификаторе, лечите генетические болезни и раздавайте полезные инъекторы по СРП.
